...
{
    public partial class frmAssignTokenStreams : Form
    {
        ...
        private void FillStreamData()
        {
            ...
            string str = "";
            str = "select *  from tbStreaming where dealercode ='" + cmbLType.Text + "' and dfclientid=" + Convert.ToInt32(cmbStreamOwner.SelectedValue) + " order by streamName";
            //
            DataTable dtDetail = new DataTable();
            InitilizeStreamGrid();
            ...
            if (dtDetail.Rows.Count > 0)
            {
                for (int i = 0; i <= dtDetail.Rows.Count - 1; i++)
                {
                    ...
                    dgStream.Rows[dgStream.Rows.Count - 1].Cells["sname"].Value = dtDetail.Rows[i]["streamName"].ToString();
                }
            }
        ...
        Int32 StreamOwnerId = 0;
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (SubmitValidation() == false) return;
            StreamOwnerId = Convert.ToInt32(cmbStreamOwner.SelectedValue);
            SaveData();
                ...
                if (Convert.ToBoolean(dgStream.Rows[iStream].Cells[1].Value) == true)
                {
                    for (int iToken = 0; iToken < dgToken.Rows.Count; iToken++)
                    {
                        if (Convert.ToBoolean(dgToken.Rows[iToken].Cells[1].Value) == true)
                        {
                            ...
                            cmdOnline.CommandText = "insert into tbAssignStreamToken (tokenid,streamid,StreamOwnerClientid) values (" + dgToken.Rows[iToken].Cells[0].Value + "," + dgStream.Rows[iStream].Cells[0].Value + "," + StreamOwnerId + ")";
        ...
        private void dgStreamUpdate_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 2)
            {
                ...
                {
                    string strDel = "";
                    strDel = "delete from tbAssignStreamToken where tokenid=" + Convert.ToInt32(cmbTokenUpdate.SelectedValue) + " ";
        ...
        private void FillStreamDataUpdate()
        {
            ...
            string str = "";
            str = "select distinct tbStreaming.*  from tbStreaming ";
            str = str + " inner join tbAssignStreamToken on tbStreaming.streamid= tbAssignStreamToken.streamId ";
            str = str + " where tbAssignStreamToken.tokenid=" + Convert.ToInt32(cmbTokenUpdate.SelectedValue);
            str = str + " order by tbStreaming.streamName ";
            DataTable dtDetail = new DataTable();
            ...
            if (dtDetail.Rows.Count > 0)
            {
                for (int i = 0; i <= dtDetail.Rows.Count - 1; i++)
                {
                    ...
                    dgStreamUpdate.Rows[dgStreamUpdate.Rows.Count - 1].Cells["sname"].Value = dtDetail.Rows[i]["streamName"].ToString();
                }
            }
        ...
        private void btnRefersh_Click(object sender, EventArgs e)
        {
            ...
            cmbStreamOwner.SelectedValue = 0;
            cmbLType.Text = "";
        }
