...
namespace ManagementPanel
{
    public partial class frmPrayer : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection con = new OleDbConnection(conStr))
            {
                using (OleDbCommand cmd = new OleDbCommand())
                {
                    using (OleDbDataAdapter oda = new OleDbDataAdapter())
                    {
                        ...
                        dgExcel.Columns[3].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                        dgExcel.Columns[4].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                        dgExcel.Columns[5].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            ...
            if (CityId != "")
            {
                strD3 = "insert into PrayerDelete select distinct prayerid from tbprayer where month(startdate)="+ string.Format("{0:MM}",Convert.ToDateTime(dt.Rows[0][0])) + " and  prayerid in (select prayerid from tbprayerdetail where cityid in(" + CityId + "))";
            ...
            //  if (stateId != "")
            //{
            //    strD3 = "insert into PrayerDelete select distinct prayerid from tbprayer where month(startdate)=" + string.Format("{0:MM}", Convert.ToDateTime(dt.Rows[0][0])) + " and prayerid in (select prayerid from tbprayerdetail where stateid in(" + stateId + "))";
            //    strD1 = "delete from tbprayer where prayerid in (select prayerid from PrayerDelete)";
            //    strD2 = "delete from tbprayerdetail where prayerid in (select prayerid from PrayerDelete)";
            //    if (StaticClass.constr.State == ConnectionState.Open) { StaticClass.constr.Close(); }
            //    StaticClass.constr.Open();
            ...
            //  if (CountryId != "")
            //{
            //    strD3 = "insert into PrayerDelete select distinct prayerid from tbprayer where month(startdate)=" + string.Format("{0:MM}", Convert.ToDateTime(dt.Rows[0][0])) + " and prayerid in (select prayerid from tbprayerdetail where CountryId in(" + CountryId + "))";
            //    strD1 = "delete from tbprayer where prayerid in (select prayerid from PrayerDelete)";
            //    strD2 = "delete from tbprayerdetail where prayerid in (select prayerid from PrayerDelete)";
            //    if (StaticClass.constr.State == ConnectionState.Open) { StaticClass.constr.Close(); }
            //    StaticClass.constr.Open();
        ...
        private Boolean SubmitValidation()
        {
            ...
            {
                MessageBox.Show("Please select a excel file", "Management Panel");
                picBrowse.Focus();
                return false;
            }
            if (txtDuration.Text == "")
            {
                MessageBox.Show("Prayer time duration cannot be blank", "Management Panel");
                txtDuration.Focus();
                return false;
            }
        ...
        private void InitilizeAdvertisement(DataGridView dgGrid)
        {
            ...
            //dgGrid.Columns[7].AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
        }
        ...
        {
            InitilizeAdvertisement(dgPrayer);
        }
        ...
        private void dgPrayer_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 6)
            {
                ...
                Localstr = Localstr + "	where PrayerId= " + Convert.ToInt32(dgPrayer.Rows[e.RowIndex].Cells["pId"].Value);
                DataTable dtDetail;
                DataTable dtCommon;
            ...
            if (e.ColumnIndex == 7)
            {
                ...
                DialogResult result;
                if (dgPrayer.Rows.Count <= 0) return;
                if (Convert.ToInt32(dgPrayer.Rows[e.RowIndex].Cells[0].Value) != 0)
                {
                    ...
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        cmd.CommandText = "delete from tbPrayerDetail where Prayerid=" + Convert.ToInt32(dgPrayer.Rows[e.RowIndex].Cells[0].Value);
                        cmd.ExecuteNonQuery();
                        StaticClass.constr.Close();
                        ...
                        cmd2.CommandText = "delete from tbPrayer where Prayerid=" + Convert.ToInt32(dgPrayer.Rows[e.RowIndex].Cells[0].Value);
                        cmd2.ExecuteNonQuery();
                        StaticClass.constr.Close();
                        dgPrayer.Rows.RemoveAt(e.RowIndex);
                    }
                }
                ClearData();
            }
        ...
        private void bgWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dgExcel.Rows)
            {
                ...
                using (SqlCommand cmd = new SqlCommand("INSERT INTO tbPrayerExcel(Date, Day, [Fajr AM],[Dhuhur PM], [Asr PM], [Magrib PM],[Isha PM]) VALUES(@Date, @Day, @Fajr,@Dhuhur, @Asr, @Magrib,@Isha)", StaticClass.constr))
                {
                    ...
                    {
                        cmd.Parameters.AddWithValue("@Date", row.Cells[0].Value);
                        cmd.Parameters.AddWithValue("@Day", row.Cells[1].Value);
                        cmd.Parameters.AddWithValue("@Fajr", row.Cells[2].Value);
                        cmd.Parameters.AddWithValue("@Dhuhur", row.Cells[3].Value);
                        cmd.Parameters.AddWithValue("@Asr", row.Cells[4].Value);
                        cmd.Parameters.AddWithValue("@Magrib", row.Cells[5].Value);
                        cmd.Parameters.AddWithValue("@Isha", row.Cells[6].Value);
                        if (StaticClass.constr.State == ConnectionState.Open) StaticClass.constr.Close();
                        StaticClass.constr.Open();
                        cmd.ExecuteNonQuery();
                        StaticClass.constr.Close();
                ...
                if (dtM.Rows.Count > 0)
                {
                    for (int iRow = 0; (iRow <= (dtM.Rows.Count - 1)); iRow++)
                    {
                        ...
                        for (int iCol = 2; (iCol <= (dtM.Columns.Count - 1)); iCol++)
                        {
                            ...
                            cmd.Parameters["@EndTime"].Value = Convert.ToDateTime(string.Format("{0:hh:mm tt}", dtM.Rows[iRow][iCol])).AddMinutes(Convert.ToInt32(txtDuration.Text));
        ...
        private void picSearch_Click(object sender, EventArgs e)
        {
            ...
            FillData(dgPrayer, "spPrayerData_AdminPanel " + dtpCurrentDate.Value.Month  + ", " + Convert.ToInt32(cmbCountryName.SelectedValue) + ", " + Convert.ToInt32(cmbStateName.SelectedValue) + " ,"+ Convert.ToInt32(cmbCityName.SelectedValue) + "");
        }
        ...
        private void picExport_Click(object sender, EventArgs e)
        {
            if (dgPrayer.Rows.Count > 0)
            {
                ...
                if (result == DialogResult.OK)
                {
                    DataTable dt = new DataTable();
                    //Adding the Columns
                    foreach (DataGridViewColumn column in dgPrayer.Columns)
                    {
